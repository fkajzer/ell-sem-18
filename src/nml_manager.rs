...
pub fn run(track_list: &Vec<Track>) {
    ...
    for track in track_list {
        file_mappings.insert(format!("{}.{}", track.file_name.clone(), track.extension.clone()), track.clone());
    };
                ...
                elem.extend_attributes(e.attributes().map(|attr| {
                    ...
                    if attr.key == b"KEY" {
                        ...
                        if file_mappings.contains_key(original_name.to_owned()) {
                            let short_name = &file_mappings.get(original_name.to_owned()).unwrap().short_name;
                            let new_file_name = format!("{}{}/:{}", value_splitted[0], nml_formatted_dir, short_name);
                        ...
                        Ok(Event::Start(ref e_inner)) if e_inner.name() == b"LOCATION" => {
                            ...
                            if file_mappings.contains_key(&file_name) {
                                ...
                                location_elem.extend_attributes(e_inner.attributes().map(|attr| {
                                    ...
                                    match attr.key {
                                        b"FILE" => attr.value = Cow::from(matching_track.short_name.as_bytes().to_vec()),
                                        b"DIR" => attr.value = Cow::from(nml_formatted_dir.as_bytes().to_vec()),
                                        _ => ()
                                    }
                                ...
                                entry_elem.extend_attributes(e.attributes().map(|attr| {
                                    ...
                                    match attr.key {
                                        b"TITLE" => attr.value = Cow::from(matching_track.title.as_bytes().to_vec()),
                                        b"ARTIST" => attr.value = Cow::from(matching_track.author.as_bytes().to_vec()),
                                        _ => ()
                                    }
